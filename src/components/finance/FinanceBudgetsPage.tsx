...
function BudgetReaderSession({userId,getToken,year,findingsOnly}: {userId:string;getToken:()=>Promise<string|null>;year:number;findingsOnly:boolean}) {
 ...
 return findingsOnly ? <><p className="break-all">Budget run: {id}. Pinned ledger: {run.dependency_run}.</p>{current.data&&!current.data.compatible ? <p role="status">Current runs are incompatible: {current.data.compatibility_reason?.code}</p>:null}<BudgetFindings key={id} findings={run.payload.derived.findings}/></> : <FinanceBudgetsView key={id} payload={run.payload} runId={id} compatibility={current.data}/>;
}
