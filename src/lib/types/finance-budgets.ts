...
export type BudgetMetric =
  "budget" | "actual" | "projected" | "variance_all" | "variance_masi";
...
export interface BudgetPayload {
  ...
  lines_by_bc: {
    bc: string | number;
    line_ids: string[];
    ledger_actual: string;
    ledger_row_count: number;
  }[];
  summary: {
    complete: boolean;
    finding_count: number;
    ledger_contributor_count: number;
...
export interface FinanceRowsPage {
  results: FinanceLedgerRow[];
  next: string | null;
  previous: string | null;
  run_id: string;
  ledger_run_id: string;
  management_accounts_sha256: string;
  contributor_basis: "full_ledger_amount_before_budget_share";
}

/** Optional read-time enrichment; original approved payload and manifest stay unchanged. */
export interface BudgetInsightMetric {
  total: string | null;
  known_subtotal: string;
  complete: boolean;
  residual: string | null;
  part_refs: { ref: string; sign: 1 }[];
}
export interface BudgetComposition {
  ...
  buckets: {
    ...
    label: string;
    amount: string | null;
    /** Percentage from exact source operands, six decimal places; null when unavailable. */
...
export interface BudgetInsights {
  ...
  accounting_year: number;
  sheet_as_of: string;
  organisation: Record<BudgetMetric, BudgetInsightMetric>;
